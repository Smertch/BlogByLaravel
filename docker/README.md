...
## Docker на Linux: нет сокета / демон не отвечает

Сообщения вида `no such file or directory` для **`~/.docker/desktop/docker.sock`** или **`/var/run/docker.sock`** значат одно: **клиент Docker не видит запущенный демон** по этому пути. Нужно либо **запустить демон**, либо **включить контекст с правильным сокетом**.

### 1. Понять, что у вас установлено

- **Только Docker Desktop для Linux** — демон стартует вместе с приложением *Docker Desktop*. Пока оно не запущено, сокета нет.
- **Пакет `docker.io` / Docker Engine (CE)** из репозитория дистрибутива — демон это сервис **`docker`**.

Проверка сокетов:

```bash
ls -la /var/run/docker.sock ~/.docker/desktop/docker.sock 2>/dev/null
docker context ls
```

### 2. Вариант A: Docker Engine (docker-ce / пакет из Ubuntu)
...
Пользователь в группе `docker` (иначе нужен `sudo` для клиента):
...
Контекст на системный сокет:

```bash
docker context create blog-engine --docker "host=unix:///var/run/docker.sock"
docker context use blog-engine
docker info
```

### 3. Вариант B: Docker Desktop

1. Запустите **Docker Desktop** из меню приложений и дождитесь статуса *running*.
2. Посмотрите контексты: `docker context ls`. Обычно есть **`desktop-linux`** — переключитесь на него:
...
3. Контекст **`blog-engine`** с `unix:///var/run/docker.sock` для Desktop **не подходит**, если демон Desktop не пробрасывает engine в этот путь (часто так и есть).

### 4. Оба варианта не ставились вместе?

Если установлен только CLI/плагины без демона — `docker info` всегда будет падать. Нужно установить **либо** Docker Engine, **либо** Docker Desktop и **запустить** соответствующий демон.

Официальные инструкции: [Install Docker Engine on Ubuntu](https://docs.docker.com/engine/install/ubuntu/), [Docker Desktop on Linux](https://docs.docker.com/desktop/install/linux/).

[Документация по contexts](https://docs.docker.com/engine/manage-resources/contexts/).
...
-------|--------------------------
Webserver|[localhost:1515](http://localhost:1515)
Mailhog web interface|[localhost:1516](http://localhost:1516)
MySQL|**host:** `localhost`; **port:** `1517`
